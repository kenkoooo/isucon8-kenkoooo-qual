...
func main() {
	...
	fp, err := os.OpenFile("./echo.log", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		panic(err)
	}
	e.Use(middleware.LoggerWithConfig(middleware.LoggerConfig{Output: fp}))
	e.Static("/", "public")
	...
	e.GET("/admin/api/reports/events/:id/sales", getSalesById, adminLoginRequired)
...
func renderReportCSV(c echo.Context, reports []Report) error {
	sort.Slice(reports, func(i, j int) bool { return reports[i].SoldAtUnix < reports[j].SoldAtUnix })
